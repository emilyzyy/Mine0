// Persistent JARVIS executor CLI.
//
// Requires the worker already running on RunPod:
//   npm run jarvis:worker start
//
// Then run with or without objective:
//   npm run jarvis:persistent
//   npm run jarvis:persistent -- "Mine me some iron"
//
// Env vars:
//   JARVIS_PERSISTENT_RESET_ON_START=0   reuse the existing Minecraft session
//   JARVIS_MAX_FRAMES=5                  frames per /run_goal call
//   MINE0_MAX_DECISION_STEPS=1           planner loop limit (floor: 1 for this CLI)
...
// Read MINE0_MAX_DECISION_STEPS directly with a floor of 1 (not 25) so
// short JARVIS demo runs like MINE0_MAX_DECISION_STEPS=1 are honoured.
// Default is 1 — each persistent /run_goal call costs ~60-90 s of real time.
const rawSteps = process.env["MINE0_MAX_DECISION_STEPS"];
const maxDecisionSteps = Math.max(1, rawSteps ? Number(rawSteps) || 1 : 1);
...
console.log(`  max decisions   : ${maxDecisionSteps}  (MINE0_MAX_DECISION_STEPS, floor=1)`);
console.log(`  objective       : ${objective}`);
if (plannerConfig.provider === "mock") {
  console.log("  *** WARNING: CEREBRAS_API_KEY not set — running with mock planner ***");
}
console.log("---");
